...
export default function About() {
  ...
  const isTriangleInView = useInView(triangleRef, { once: true, margin: "-80px" });
    ...
    <section ref={dividerRef} id="nosotros" className="py-16 md:py-24 divider-reveal">
      <div className="grid grid-cols-1 gap-10 md:grid-cols-12 md:gap-12">
        ...
        <div className="relative flex flex-col items-start justify-center overflow-hidden md:col-span-4 md:items-center">
          ...
          <motion.div
            ref={triangleRef}
            className="pointer-events-none absolute inset-0 -m-10 sm:-m-12 md:-m-14 md:-ml-20"
            initial={{ opacity: 0 }}
            animate={isTriangleInView ? { opacity: 0.12 } : { opacity: 0 }}
            transition={{ duration: 1.5, delay: 0.3, ease: [0.25, 1, 0.5, 1] }}
          >
            <svg
              viewBox="0 0 100 100"
              className="w-full h-full stroke-brand fill-none stroke-[0.4]"
